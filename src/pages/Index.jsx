...
export default function Index() {
  ...
  const girdRefCallbackCol1 = useCallback(
    (node) => {
  ...
  const girdRefCallbackCol2 = useCallback(
    (node) => {
  ...
  const girdRefCallbackCol3 = useCallback(
    (node) => {
  ...
  const desktopLayout = (
    <div ref={layoutRef} css={desktopLayoutStyle}>
      <div ref={girdRefCallbackCol1}>
        {!gridDatas[0]
          ? null
          : gridDatas[0].map((data, index) => (
              <Content key={index} datas={data} />
            ))}
      </div>
      <div ref={girdRefCallbackCol2}>
        {!gridDatas[1]
          ? null
          : gridDatas[1].map((data, index) => (
              <Content key={index} datas={data} />
            ))}
      </div>
      <div ref={girdRefCallbackCol3}>
        {!gridDatas[2]
          ? null
          : gridDatas[2].map((data, index) => (
              <Content key={index} datas={data} />
            ))}
      </div>
    </div>
  ...
  const mobileLayout = (
    <div ref={layoutRef}>
      {bookmarks
        .filter((value) => value.content.includes(searchQuery))
        .map((contents, index) => (
          <Content key={index} datas={contents} />
        ))}
    </div>
  );
  ...
  const getBookmarksAndUpdate = (maxReqCount) => {
    ...
    axios
      .get("http://localhost:7777/bookmark/testuser01/", config)
      .then((res) => {
  ...
  useEffect(() => {
    if (
      typeof bookmarks[renderedCount] === "undefined" ||
      window.innerWidth <= 900
    ) {
      return;
    }
    ...
    switch (gridSizes.indexOf(Math.min(...gridSizes))) {
      case 0:
        setGridDatas([
          [...gridDatas[0], bookmarks[renderedCount]],
          [...gridDatas[1]],
          [...gridDatas[2]],
        ]);
        setReneredCount(renderedCount + 1);
      ...
      case 1:
        setGridDatas([
          [...gridDatas[0]],
          [...gridDatas[1], bookmarks[renderedCount]],
          [...gridDatas[2]],
        ]);
        setReneredCount(renderedCount + 1);
        break;
      case 2:
        setGridDatas([
          [...gridDatas[0]],
          [...gridDatas[1]],
          [...gridDatas[2], bookmarks[renderedCount]],
        ]);
        setReneredCount(renderedCount + 1);
        break;
      default:
